...
class HttpEventStreamActorTest extends TestKit(ActorSystem())
  ...
  before {
    ...
    streamActor = TestActorRef(Props(
      new HttpEventStreamActor(stream)
    ))
  }
  ...
  test("Register Handler") {
    ...
    verify(stream).subscribe(streamActor.underlyingActor.clients(handle), classOf[MarathonEvent])
  }
  ...
  test("Unregister an already registered Handler") {
    ...
    val actor = streamActor.underlyingActor.clients(handle)
